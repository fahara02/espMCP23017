...
class InterruptManager
{
  ...
  public:
	...
	explicit InterruptManager(MCP::MCP_MODEL m, I2CBus& bus, std::shared_ptr<MCP::Register> iconA,
	...
	template<typename T>
	void registerCallback(MCP::PORT port, uint8_t pin, void (*func)(T*), T* data)
	{
	...
	template<typename T>
	void attachMainHandler(PORT port, gpio_num_t espPin, void (*func)(void*) = nullptr,
						   T* userData = nullptr)
	{
		if(port == PORT::GPIOA)
		{
  ...
  private:
	...
	void setupISR(gpio_num_t pin, ISRHandlerData& handlerData, gpio_int_type_t mode,
